...
export class GalleryView extends GalleryViewCommon {
  ...
  createNativeView(): UIView {
    ...
    (this.nativeView as UIView).backgroundColor = UIColor.redColor;
  ...
  private async renderUI() {
    try {
      ...
      // const imagenes = await this.mediaStore.getImagenes();
      const imagenes = await this.mediaStore.getMedia();

      // this.files = imagenes as Array<IFiles>;
      this.files = new ObservableArray<IFiles>(imagenes);
      this.files.getItem(0).isSelected = true;
