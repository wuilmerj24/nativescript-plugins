...
export function getCurrentPais(addPlus: boolean): Promise<any> {
  return new Promise((rs, rj) => {
    ...
    if (checkPermisos()) {
      let context: android.content.Context = Application.android.foregroundActivity || Utils.android.getApplicationContext();
...
function checkPermisos(): any {
  ...
  for (let i: number = 0; i < permisos.length; i++) {
    if (android.os.Build.VERSION.SDK_INT < 23) {
      permisos_respuesta.push((Application.android.foregroundActivity || (Utils.android.getApplicationContext() as android.content.Context)).checkPermission(permisos[i], android.os.Process.myPid(), android.os.Process.myUid()) === android.content.pm.PackageManager.PERMISSION_GRANTED ? 'authorized' : 'denied');
    } else {
      permisos_respuesta.push((Application.android.foregroundActivity || (Utils.android.getApplicationContext() as android.content.Context)).checkSelfPermission(permisos[i]));
    }
  }
