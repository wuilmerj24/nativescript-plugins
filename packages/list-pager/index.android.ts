...
export class ListPager extends ListPagerCommon {
    ...
    public createNativeView() {
        ...
        const layoutManager: androidx.recyclerview.widget.LinearLayoutManager = new androidx.recyclerview.widget.LinearLayoutManager(this._context);
        ...
        nativeView.setLayoutDirection(androidx.recyclerview.widget.LinearLayoutManager.HORIZONTAL);
        if (sdkVersion >= 21) {
    ...
    initStaticPagerAdapter() {
        if (!this.items && this._childrenCount > 0) {
            ...
            const nativeView = this.nativeViewProtected;
            if (nativeView && !(this._listPagerAdapter instanceof StaticPagerStateAdapter)) {
                this._listPagerAdapter = new StaticPagerStateAdapter(new WeakRef(this));
                nativeView.setAdapter(this._listPagerAdapter);
...
interface PagerViewHolder extends androidx.recyclerview.widget.RecyclerView.ViewHolder {
    // tslint:disable-next-line:no-misused-new
    new(owner: View, pager: WeakRef<ListPager>): PagerViewHolder;
    view: View;
}
...
function initPagerViewHolder() {
    ...
    class PagerViewHolderImpl extends androidx.recyclerview.widget.RecyclerView.ViewHolder {
        constructor(
            private owner: View,
            private pager: WeakRef<ListPager>
        ) {
